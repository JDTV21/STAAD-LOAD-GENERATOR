...
with c3:
    ...
    st.metric("Building Height (m)", building_height)
...
with c3:
    partition = st.number_input("Partition Allowance (kPa)", value=1.0)
...
st.divider()
st.header("Live Load (NSCP Table 205-1)")
...
roof_type = st.selectbox("Roof Finish", roof_loads.keys())

roof_DL = roof_loads[roof_type]
...
with c3:
    distance = st.number_input("Distance to Active Fault (km)", value=5.0)
...
system = st.selectbox("Frame System",
["RC Moment Frame","Steel Moment Frame","Braced Frame","Shear Wall"])
...
# -------------------------------------------------
# LOAD SUMMARY (RESTORED)
# -------------------------------------------------

st.divider()
st.header("LOAD SUMMARY FOR STAAD")
...
with c2:
    st.metric("Live Load (kPa)",LL)

with c3:
    st.metric("Roof Dead Load (kPa)",roof_DL)

with c4:
    st.metric("Wall Load (kN/m)",round(wall_load,3))
...
with c4:
    st.metric("Base Shear",round(V,3))

# -------------------------------------------------
# STAAD COMMAND OUTPUT
# -------------------------------------------------
